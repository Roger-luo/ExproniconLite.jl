begin
    #= none:1 =# Core.@doc "    is_valid_typevar(typevar)\n\nCheck if the given typevar is a valid typevar.\n\n!!! note\n    This function is based on [this discourse post](https://discourse.julialang.org/t/what-are-valid-type-parameters/471).\n" function is_valid_typevar(typevar)
            let
                ...
                if x_1 isa TypeVar
                    ...
                    $(Expr(:symbolicgoto, Symbol("##final#685_1")))
                end
                if x_1 isa Symbol
                    return_1 = let
                            true
                ...
                $(Expr(:symboliclabel, Symbol("##final#685_1")))
                return_1
            end
        end
    ...
    #= none:126 =# Core.@doc "    is_function(def)\n\nCheck if given object is a function expression.\n" function is_function(#= none:131 =# @nospecialize(def))
            let
                cache_1 = nothing
                return_2 = nothing
                x_2 = def
                if x_2 isa Expr
                    if begin
                        ...
                        $(Expr(:symbolicgoto, Symbol("##final#689_1")))
                    end
                    if begin
                                x_5 = cache_1.value
                ...
                $(Expr(:symboliclabel, Symbol("##final#689_1")))
                return_2
            end
        end
    #= none:141 =# Core.@doc "    is_kw_function(def)\n\nCheck if a given function definition supports keyword arguments.\n" function is_kw_function(#= none:146 =# @nospecialize(def))
            ...
            let
                ...
                if x_9 isa Expr
                    if begin
                        ...
                        $(Expr(:symbolicgoto, Symbol("##final#700_1")))
                    end
                    if begin
                                x_15 = cache_2.value
                ...
                $(Expr(:symbolicgoto, Symbol("##final#700_1")))
                (error)("matching non-exhaustive, at #= none:154 =#")
                $(Expr(:symboliclabel, Symbol("##final#700_1")))
                return_3
            end
        end
    #= none:162 =# @deprecate is_kw_fn(def) is_kw_function(def)
    #= none:163 =# @deprecate is_fn(def) is_function(def)
    #= none:165 =# Core.@doc "    is_struct(ex)\n\nCheck if `ex` is a struct expression.\n" function is_struct(#= none:170 =# @nospecialize(ex))
            ex isa Expr || return false
            return ex.head === :struct
        end
    #= none:175 =# Core.@doc "    is_struct_not_kw_struct(ex)\n\nCheck if `ex` is a struct expression excluding keyword struct syntax.\n" function is_struct_not_kw_struct(ex)
    ...
    #= none:188 =# Core.@doc "    is_ifelse(ex)\n\nCheck if `ex` is an `if ... elseif ... else ... end` expression.\n" function is_ifelse(#= none:193 =# @nospecialize(ex))
            ex isa Expr || return false
            return ex.head === :if
        end
    #= none:198 =# Core.@doc "    is_for(ex)\n\nCheck if `ex` is a `for` loop expression.\n" function is_for(#= none:203 =# @nospecialize(ex))
            ex isa Expr || return false
            return ex.head === :for
        end
    #= none:208 =# Core.@doc "    is_field(ex)\n\nCheck if `ex` is a valid field expression.\n" function is_field(#= none:213 =# @nospecialize(ex))
            let
                cache_5 = nothing
                return_4 = nothing
                x_23 = ex
                ...
                $(Expr(:symbolicgoto, Symbol("##final#720_1")))
                (error)("matching non-exhaustive, at #= none:214 =#")
                $(Expr(:symboliclabel, Symbol("##final#720_1")))
                return_4
            end
        end
    #= none:223 =# Core.@doc "    is_field_default(ex)\n\nCheck if `ex` is a `<field expr> = <default expr>` expression.\n" function is_field_default(#= none:228 =# @nospecialize(ex))
            let
                cache_7 = nothing
                return_5 = nothing
                x_40 = ex
    ...
    #= none:253 =# Core.@doc "    is_matrix_expr(ex)\n\nCheck if `ex` is an expression for a `Matrix`.\n" function is_matrix_expr(#= none:258 =# @nospecialize(ex))
